...
** 기타 긴급 지시항으로 현재 코덱스는 컨테이너에서 내부 테스트도 진행하는데 테스트는 사용자의 로컬에서만 진행할것임. 이유는 보다 빠른 작업진행을 위해서임.
   로컬에는 모든 라이브러리가 완비 되어있기 때문에 모든 테스트가 가능함. 코덱스는 정석대로 코딩만 진행해주고 테스트는 사용자에게 일임할것.
